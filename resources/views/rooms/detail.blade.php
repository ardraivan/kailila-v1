...
@section('breadcrumb')
    <li class="breadcrumb-item"><a style="color: #C01A5B" href="{{ route('rooms.index') }}">Rooms</a></li>
    ...
    <div class="container">
                        ...
                        <thead>
                            ...
                            <tr style="border: 1px solid {{ $textColor }};">
                                ...
                                <th
                                    ...
                                    Last Update</th>
                                <th
                                    style="border: 1px solid {{ $textColor }}; width: 17%; background-color: {{ $room->colour->hexcode }}; color: {{ $textColor }}">
                                    Action</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr style="border: 1.2px solid #000;">
                                    ...
                                    <td style="border: 1.2px solid #000;">
                                        <div class="btn-group">
                                            ...
                                            @endif
                                            @php
                                                // Cek apakah quantity item untuk ruangan ini adalah nol
                                                $isQuantityZero = $itemQuantities[$item->name] === 0;
                                            @endphp
                                            <button type="button"
                                                class="btn btn-sm btn-secondary rounded dropdown-toggle ml-2"
                                            ...
                                            <div class="dropdown-menu">
                                                @if ($isQuantityZero)
                                                    <a class="dropdown-item"
                                                        href="{{ route('item_locations.createWithDefaultData', ['item_id' => $item->id, 'storage_id' => $storage_id]) }}">
                                                        Add this item
                                                    </a>
                                                @else
                                                    <a class="dropdown-item"
                                                        href="{{ route('item_locations.edit', ['item_location' => $item->itemLocations->where('item_id', $item->id)->where('storage_id', $room->id)->first()->id]) }}">
                                                        Edit quantity
                                                    </a>
                                                @endif

                                                <a class="dropdown-item"
                                                    href="{{ route('items.edit', ['item' => $item->id]) }}">Edit item
                                                    details</a>
                                            </div>
                                        </div>
        ...
        <div class="card-footer clearfix">
            ...
            <div class="float-right">
                <a href="{{ route('rooms.exportPDF', ['room_id' => $room->id, 'search' => request()->query('search')]) }}"
